...
describe('CameraController — state machine', () => {
  ...
  it('drops LOCKED flip mode to FOLLOW when the locked sightline is obstructed', () => {
    ...
    const bottle = makeBottleFixture(new THREE.Vector3(0, 0, 0.5), new THREE.Vector3(0, -1, 0));
    ctrl.activeCamera.position.set(0, -5, 2);
    ctrl._currentLookAt.set(0, 0, 0.5);

    ctrl.setStateFlip(bottle, new THREE.Vector3(3, 0, 0));
    expect(ctrl.flipMode).toBe(FLIP_MODE.LOCKED);
  ...
  it('obstacle sweep: re-evaluates continuously when a new blocker appears', () => {
    const scene = new THREE.Scene();
    const ortho = new THREE.OrthographicCamera(-1, 1, 1, -1, -10, 100);
    const persp = new THREE.PerspectiveCamera(35, 1, 0.1, 100);
    ortho.up.set(0, 0, 1);
    ...
    expect(ctrl._targetCameraAxis.dot(new THREE.Vector3(0, -1, 0))).toBeGreaterThan(0.99);
    ...
    for (let i = 0; i < 5; i++) ctrl.update(0.05, bottle);

    expect(ctrl._targetCameraAxis.dot(new THREE.Vector3(0, -1, 0))).toBeLessThan(0.99);
  });
